...
Resampling methods are commonly used for inference on the area under the ROC curve. Confidence intervals for the AUC can be obtained using bootstrapping or cross-validation [@Noma_Matsushima_Ishii_2021; @Gu_Ghosal_Roy_2008; @LeDell_Petersen_Van_Der_Laan_2015]. Permutation tests enable the assessment of statistical significance, either under the null hypothesis of no discriminatory power or when comparing the performance of two models [@Pauly_Asendorf_Konietschke_2016; @Venkatraman_2000; @Bandos_Rockette_Gur_2006]. Resampling procedures are essential for evaluating uncertainty, model stability, and statistical significance. However, despite their favorable statistical properties, including robustness [@Simonoff_Hochberg_Reiser_1986; @Pauly_Asendorf_Konietschke_2016], these methods rely on repeated recalculation of the AUC and can therefore impose considerable computational burdens, particularly for large datasets or complex models [@LeDell_Petersen_Van_Der_Laan_2015].

AUC maximization is another example of computational challenge that has recently gained importance in the development of ranking and classification algorithms. Directly optimizing AUC, rather than surrogate losses, leads to improved generalization and discrimination capabilities in large-scale settings [@Yuan_Yan_Sonka_Yang_2021; @Zhang_Xu_2022; @LeDell_etal_2016]. Unlike many other objectives, the AUC function is non-decomposable over individual examples, which complicates the use of standard optimization techniques [@Zhou_Ying_Skiena_2020]. A comprehensive overview of the last 20+ years of AUC maximization research, emphasizing its relevance for imbalanced learning, large datasets, and deep learning applications has been provided by @Yang_Ying_2023. Many, if not most, approaches to AUC maximization require repeated multiple computations of the AUC statistic [see, for example @Calders_Jaroszewicz_2007; @Yang_Zhou_Lei_Ying_2020; @Ataman_Street_Zhang_2006; @Gajowniczek_Zabkowski_2021]. 

Efficient AUC computation is also a prerequisite for many feature selection and assessment techniques. Permutation-based variable importance measures were, for example, proposed by @Janitza_Strobl_Boulesteix_2013 or @Biecek_2018, while AUC-based feature selection algorithms were, inter alia, presented by S@Sun_Wang_Wei_2017; @Su_Du_Wang_Wei_Liu_2022, @Xu_Suzuki_2014 or @Ribeiro_2021. Other applications of repeated AUC computation arise in simulation studies. For instance, @Kochanski_2021 introduced a simulation model for risk and pricing competition in which each bank optimizes its credit scoring strategy, requiring numerous AUC evaluations to assess performance under varying market conditions.
...
The rank-based formula for AUC is directly based on the method developed for the Mann–Whitney U statistic computation [see @Sheskin_2020]:
...
The computational time required to evaluate the AUC metric varies substantially across the examined implementations, spanning several orders of magnitude. For instance, the *bigstatsr* implementation computes the AUC nearly two orders of magnitude faster than the corresponding function in the *scorecard* package. This discrepancy may be attributable to several factors, including additional computations performed internally by some functions (e.g., estimation of confidence intervals or simultaneous evaluation of multiple performance metrics), differences in algorithmic design, or a combination of these effects.

Overall, *bigstatsr* consistently provides the fastest AUC computation. Although *MLMetrics* and *Hmisc* exhibit slightly lower execution times on the smallest dataset, *bigstatsr* clearly outperforms these alternatives as the number of observations increases.

Consequently, when an existing R implementation is preferred, we recommend the use of the *AUC* function from the *bigstatsr* package for efficient computation of the AUC metric, particularly in large-scale settings.
