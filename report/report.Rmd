...
*Receiver Operating Characteristic* [@Junge_Dettori_2024]. Today, the
...
different cut-off points” [@Gromada_2016], illustrating the model's
...
classifier's varying cut-off point [@Fawcett_2006].
...
desirable [@Hanley_McNeil_1982] – this is equivalent to a larger Area
...
referred to as the *c-statistic* (or concordance statistic). This term
defined by Harrell et al. [@Harrell_Califf_Pryor_Lee_Rosati_1982] and
further elaborated by, for example Austin and Steyerberg
[@Austin_Steyerberg_2012], represents the probability of concordance
...
as the *probability of superiority* indicating the likelihood that a
...
*measure of stochastic superiority* (denoted as $A$)
[@Vargha_Delaney_2000] generalized CLES concept so that it could be
...
for longitudinal data the *relative treatment effect* was defined
[@Brunner_Munzel_Puri_2002]. This metric estimates the probability that
...
[@Hand_Till_2001]: $$\text{Gini} = 2 \cdot \text{AUC} - 1$$ Cliff [1993]
...
reverse. This statistic of ordinal dominance, later named *Cliff’s
Delta* [@Bais_Van_Der_Neut_2022] was described as a non-parametric
...
which links it directly to AUC [@Newson_2002].
...
referred to as the *pseudo Gini* index to differentiate it from the
...
(referred to as Accuracy Rate) for any scoring model [@Idczak_2019].
...
[@Kochański_2022]. Such simulation analyses often involve generating
...
with a different number of observations, all of which have been
...
While *MLMetrics* and and *Hmisc* are slightly faster on the smallest
data set, *bigstatsr* noticeably outperforms them as the number of
observations increases.

It is therefore recommended to use the *AUC* function from the
*bigstatsr* package whenever calculating the AUC metric in R.
