...
as the \emph{c-statistic} (or concordance statistic). This term defined by
Harrell et al. \citep{Harrell_Califf_Pryor_Lee_Rosati_1982} and further elaborated
by, for example Austin and Steyerberg \citep{Austin_Steyerberg_2012}, represents the
...
the \emph{probability of superiority} indicating the likelihood that
...
on normal distributions, the later proposed \emph{measure of stochastic superiority}
(denoted as \(A\)) \citep{Vargha_Delaney_2000} generalized CLES
...
non-parametric analysis for longitudinal data the \emph{relative treatment effect}
was defined \citep{Brunner_Munzel_Puri_2002}. This metric estimates the probability that
...
{[}Brunner\_Munzel\_Puri\_2002{]}, \citep{Engelmann_Hayden_Tasche_2003}.
...
transformation of the AUC, defined by the relationship \citep{Hand_Till_2001}:
...
This statistic of ordinal dominance, later named \emph{Cliff's Delta}
\citep{Bais_Van_Der_Neut_2022} was described as a non-parametric effect size based
...
\citep{Hanley_McNeil_1982}, \citep{Bamber_1975}.
...
directly to AUC \citep{Newson_2002}.
...
as the \emph{pseudo Gini} index to differentiate it from the classical Gini index
...
for any scoring model \citep{Idczak_2019}.
...
estimate can be computationally expensive \citep{LeDell_Petersen_Van_Der_Laan_2015}.
...
\citep{Noma_Matsushima_Ishii_2021}. Their approach involves computing AUC estimates
...
\citep{Bandos_Rockette_Gur_2006}. Furthermore, Pauly, Asendorf,
...
single-pass metrics \citep{Janitza_Strobl_Boulesteix_2013}.
...
\citep{Biecek_2018} implements a permutation-based variable importance method
...
that is ``yet to be built'' \citep{Kochański_2022}. Such simulation analyses often
...
\citep{Kochanski_2021}. The study demonstrates that even marginal improvements in
...
\pandocbounded{\includegraphics[keepaspectratio]{report_files/figure-latex/unnamed-chunk-4-1.pdf}}

In general, \emph{bigstatsr} offers the fastest way of computation overall. While \emph{MLMetrics} and and \emph{Hmisc} are slightly faster on the smallest data set, \emph{bigstatsr} noticeably outperforms them as the number of observations increases.

It is therefore recommended to use the \emph{AUC} function from the \emph{bigstatsr} package whenever calculating the AUC metric in R.
