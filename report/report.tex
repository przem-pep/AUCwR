...
\begin{table}
\centering
\caption{\label{tab:unnamed-chunk-1}Table 1: Functions computing AUC.}
\centering
...
\hline
effsize & VD.A & VD.A(pred \textasciitilde{} factor(target))\$estimate & Rank sum & R\\
\hline
...
\hline
mltools & auc\_roc & auc\_roc(-pred, target) & Trapezoidal rule & R\\
\hline
...
\hline
scikit-learn & roc\_auc\_score & import("sklearn.metrics")\$roc\_auc\_score(target, -pred) & Trapezoidal rule & Python, C\\
\hline
scorecard & perf\_eva & perf\_eva(-pred, target, binomial\_metric = "auc", show\_plot = FALSE)\$binomial\_metric\$dat\$AUC & Trapezoidal rule & R\\
\hline
yardstick & roc\_auc\_vec & roc\_auc\_vec(as.factor(target), pred) & Trapezoidal rule & R\\
\hline
